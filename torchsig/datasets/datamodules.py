...
class TorchSigDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        # : boolean: `self.impaired` as a string variable.
        self.clean = "impaired" if impaired else "clean"
        self.seed = seed
        self.overlap_prob = overlap_prob

        self.train_config = self._set_config(
            # : dict: TorshSig train config
            self.dataset, self.impaired, True, qa, eb_no, self.seed)
        self.val_config = self._set_config(
            # : dict: TorshSig val config
            self.dataset, self.impaired, False, qa, eb_no, self.seed)
...
class NarrowbandDataModule(TorchSigDataModule):
    ...
    def __init__(self,
        ...
        Args:
            ...
            collate_fn (Optional[Callable], optional): Dataloader custom collate function. Defaults to TorchSig collate_fn.
        """
        super().__init__(root, "Narrowband", impaired, qa, eb_no, seed, overlap_prob,
                         transform, target_transform, batch_size, num_workers, collate_fn)
    ...
    def prepare_data(self) -> None:
        ...
        creator_train = DatasetCreator(
            ds_train, seed=self.seed, path=self.train_path, num_workers=self.num_workers)
        print(f"Using {self.train_config.__name__} for train.")
        creator_train.create()

        creator_val = DatasetCreator(
            ds_val, seed=self.seed, path=self.val_path, num_workers=self.num_workers)
...
class WidebandDataModule(TorchSigDataModule):
    ...
    def __init__(self,
        ...
        Args:
            ...
            collate_fn (Optional[Callable], optional): Dataloader custom collate function. Defaults to TorchSig collate_fn.
        """
        super().__init__(root, "Wideband", impaired, qa, False, seed, overlap_prob,
                         transform, target_transform, batch_size, num_workers, collate_fn)
