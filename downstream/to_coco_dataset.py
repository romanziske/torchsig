    ...
    def convert(self, split: Literal["train", "val"]) -> Path:
        ...
        for batch_idx, (data, targets) in enumerate(tqdm(dataloader)):
            for img_idx, (img, target) in enumerate(zip(data, targets)):
                ...
                boxes = target["boxes"]  # [N, 4] tensor in format [x, y, width, height]
                for box_idx, box in enumerate(boxes):
                    x, y, w, h = box.tolist()  # Already in COCO format
                    
                    coco_dict["annotations"].append({
                        "id": ann_id,
                        "image_id": img_id,
                        "category_id": target["labels"][box_idx].item(),
                        "bbox": [x, y, w, h],  # Already normalized coordinates
                        "area": float(w * h),  # Normalized area from width*height
                        "iscrowd": 0
                    })
                    ann_id += 1
